...
class Trainer_Plan:
    ...
    def train(
        ...
        **kwargs,
        ...
        tr_pl_loss = torch.tensor(0.0).to(args.device)
        tr_acc = torch.tensor(0.0).to(args.device)
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar = 0.0
        self._total_pl_loss_scalar = 0.0 
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step, tr_pl_loss_step, tr_acc_step = self.training_step(model, inputs, step)
                else:
                    tr_loss_step, tr_pl_loss_step, tr_acc_step = self.training_step(model, inputs, step)
                ...
                if (
                    args.logging_nan_inf_filter
                    and not is_torch_tpu_available()
                    and (torch.isnan(tr_pl_loss_step) or torch.isinf(tr_pl_loss_step))
                ):
                    # if loss is nan or inf simply add the average of previous logged losses
                    tr_pl_loss += tr_pl_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                else:
                    tr_pl_loss += tr_pl_loss_step 
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, tr_pl_loss, tr_acc, model, trial, epoch, ignore_keys_for_eval)
                else:
        ...
        self._total_pl_loss_scalar += tr_pl_loss.item() 
        self._total_acc_scalar += tr_acc.item() 

        train_loss = self._total_loss_scalar / self.state.global_step
        train_pl_loss = self._total_pl_loss_scalar / self.state.global_step
        ...
        metrics["train_pl_loss"] = train_pl_loss
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_pl_loss, tr_acc, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_log:
            ...
            tr_pl_loss_scalar = self._nested_gather(tr_pl_loss).mean().item()
            tr_acc_scalar = self._nested_gather(tr_acc).mean().item()

            # reset tr_loss to zero
            tr_loss -= tr_loss
            tr_pl_loss -= tr_pl_loss
            tr_acc -= tr_acc

            logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["pl_loss"] = round(tr_pl_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["acc"] = round(tr_acc_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["learning_rate"] = self._get_learning_rate()

            self._total_loss_scalar += tr_loss_scalar
            self._total_pl_loss_scalar += tr_pl_loss_scalar
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], step) -> torch.Tensor:
        ...
        with self.autocast_smart_context_manager():
            loss, pl_loss = self.compute_loss(model, inputs, step)
                    
        if self.args.n_gpu > 1:
            loss = loss.mean()  # mean() to average on multi-gpu parallel training
            pl_loss = pl_loss.mean()
            acc = acc.mean() 
        
        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            # deepspeed handles loss scaling by gradient_accumulation_steps in its `backward`
            loss = loss / self.args.gradient_accumulation_steps
            pl_loss = pl_loss / self.args.gradient_accumulation_steps
            acc = acc / self.args.gradient_accumulation_steps

        all_loss = loss + pl_loss  #+ self.pl_loss_alpha * pos_loss 
        if self.do_grad_scaling:
        ...
        return loss.detach(), pl_loss.detach(), acc.detach() 
    ...
    def compute_loss(self, model, inputs, step, return_outputs=False):
        ...
        if step % 2 == 0:
            ...
            curr_inputs = {'input_ids': inputs['pl_input_ids'],'plan_id': inputs['pl_plan_id'], 'labels': inputs['pl_labels']}
            pl_loss = model(**curr_inputs)
